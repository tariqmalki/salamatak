...
import { restaurants, getAllDishes } from '../data/restaurants'

// Dish Checker - user inputs restaurant + dish name to check for allergens
export default function DishChecker() {
  ...
  const handleCheck = () => {
    ...
    setTimeout(() => {
      ...
      if (matchedDish) {
        // Found in database - use actual ingredients
        ...
        // Not in database - use keyword matching on the dish name itself
        const combinedText = `${restaurantName} ${dishName}`
        const allergens = detectAllergens(combinedText, selectedAllergies)

        // Also check common risky keywords
        const riskyKeywords = ['sauce', 'cream', 'special', 'secret', 'mixed', 'assorted']
  ...
  // Suggested restaurants for quick fill
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 mb-6">Enter a restaurant and dish to check if it's safe for you</p>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <button
          ...
          className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300 text-white font-bold py-4 rounded-xl transition-all text-lg cursor-pointer"
        >
        ...
        <div className={`rounded-2xl p-6 border-2 ${
          ...
          <div className="space-y-2 text-sm">
            ...
            {!result.found && (
              <p className="text-gray-500 italic">
                ⚠️ This dish was not found in our database. Results are based on keyword analysis.
              </p>
            )}
          </div>
          ...
          {result.hasRiskyKeyword && result.level === 'warning' && (
            <p className="mt-4 text-yellow-700 text-sm">
              ⚠️ This dish contains words like "sauce", "cream", or "special" which may contain hidden allergens. Please ask the restaurant staff.
            </p>
          )}
        </div>
      )}
